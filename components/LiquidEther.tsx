...
`;
    ...
    class ShaderPass {
      ...
      runPass() {
        if (this.props.output && this.scene && this.camera && Common.renderer) {
    ...
    class Advection extends ShaderPass {
      ...
      update(params: { dt: number; isBounce: boolean; BFECC: boolean }) {
        ...
        if (params.isBounce) {
          this.uniforms.boundarySpace.value.set(0, 0);
        } else {
          this.uniforms.boundarySpace.value.copy(this.uniforms.px.value);
        }
        this.runPass();
      }
    }
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(params: { cursor_size: number; mouse_force: number; cellScale: THREE.Vector2 }) {
        if (Mouse.diff.x === 0 && Mouse.diff.y === 0) return;
        const forceX = Mouse.diff.x * params.mouse_force;
        const forceY = Mouse.diff.y * params.mouse_force;
        this.uniforms.force.value.set(forceX, forceY);
        this.uniforms.center.value.copy(Mouse.coords);
        this.uniforms.scale.value.set(params.cursor_size, params.cursor_size);
        this.uniforms.px.value.copy(params.cellScale);
        this.runPass();
      }
    }
    ...
    class Simulation {
      ...
      update() {
        ...
        let vel = this.fbos.vel_1 as THREE.WebGLRenderTarget;
